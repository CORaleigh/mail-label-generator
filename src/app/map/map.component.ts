...
export class MapComponent implements OnInit, OnChanges {
   ...
   */
  private _propertyLayer: string = 'https://maps.raleighnc.gov/arcgis/rest/services/Property/Property/MapServer/0';
  ...
  async initializeMap() {
    try {
      const [EsriMap, EsriMapView, Search, FeatureLayer, FeatureLayerSearchSource, geometryEngine, Graphic] = await loadModules([
        'esri/Map',
        'esri/views/MapView',
        'esri/widgets/Search',
        ...
        search.on('select-result', event => {
          ...
          outSpatialReference: mapView.spatialReference}).then(result => {
            if (result.features.length > 0) {
              ...
              this.bufferProperty(mapView, property, result.features[0], geometryEngine, Graphic);
            }
          });
  ...
  bufferProperty (mapView, property, selectedProperty, geometryEngine, Graphic) {
     ...
     property.queryRelatedFeatures({objectIds: oids, relationshipId: 0, 
       ...
       result.features.forEach(f => {
         relates[f.attributes.OBJECTID].features.forEach(condo => {
           if (labelatts.indexOf(condo.attributes) === -1) {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (this._selectedProperty) {
      ...
      loadModules([
        'esri/geometry/geometryEngine',
        'esri/Graphic']).then(([geometryEngine, Graphic]) => {
          this.bufferProperty(this._mapView, this._property, this._selectedProperty, geometryEngine, Graphic);
        });
    }
